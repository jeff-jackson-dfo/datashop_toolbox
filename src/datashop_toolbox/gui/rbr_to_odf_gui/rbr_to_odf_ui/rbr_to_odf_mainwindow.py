...
class MainWindow(QMainWindow):
    ...
    def _save_settings(self):
        data = {
            ...
            "selected_channels": [
                item.text()
                for item in self.ui.channelListWidget.selectedItems()
            ],
    ...
    def _load_settings(self):
        ...
        if folder and Path(folder).exists():
            ...
            rsk_files = sorted(
                p.name for p in Path(folder).iterdir()
                if p.is_file() and p.suffix.lower() == ".rsk"
            )
        ...
        if self._rsk_file:
            matches = self.ui.rskListWidget.findItems(
                self._rsk_file, Qt.MatchExactly
            )
            if matches:
    ...
    def _choose_rsk_folder(self):
        ...
        folder_path = QFileDialog.getExistingDirectory(self, "Select the folder containing the RSK files", dir=".")
        if folder_path:
    ...
    def _profile_plots(self):
        ...
        station_latitude = float(self.ui.latitudeLineEdit.text()) if self.ui.latitudeLineEdit.text() else None
        station_longitude = float(self.ui.longitudeLineEdit.text()) if self.ui.longitudeLineEdit.text() else None
    ...
    def _choose_export_odf_folder(self) -> str:
        ...
        odf_folder_path = QFileDialog.getExistingDirectory(self, "Select the folder to save the ODF file(s)", dir=".")
        if odf_folder_path:
