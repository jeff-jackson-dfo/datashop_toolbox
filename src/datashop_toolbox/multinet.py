...
class MultinetHeader(OdfHeader):
    ...
    def create_sytm(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["times"] = df["time"].apply(
            lambda x: datetime.strptime(x, MultinetHeader.time_format).time()
        )
        ...
        df["sytm"] = (
            df["datetimes"]
            .apply(lambda x: datetime.strftime(x, BaseHeader.SYTM_FORMAT))
            .str.upper()
        )
    ...
    def fix_datetime(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self.check_datetime_format(df["date"].iloc[0], MultinetHeader.date_format):
            raise ValueError(
                f"Date format is incorrect. Expected format: {MultinetHeader.date_format}"
            )
        if not self.check_datetime_format(df["time"].iloc[0], MultinetHeader.time_format):
            raise ValueError(
                f"Time format is incorrect. Expected format: {MultinetHeader.time_format}"
            )
    ...
    def compute_pressure(
        pressure_raw: pd.Series, pressure_temp_raw: pd.Series, pcal: list
    ) -> float:
        """
    ...
    def populate_odf_headers(self, df: pd.DataFrame):
        ...
        self.odf_header.cruise_header.institution = (
    ...
    def read_multinet_files(self, file_type: str, file_path: str) -> pd.DataFrame:
        ...
        match file_type:
            case "raw":
                ...
                pressure = multinet.compute_pressure(
                    pressure_raw, pressure_temp_raw, multinet.calibrations["pressure"]
                )
                df["pressure"] = round(pressure, 1)

                temperature_raw = df["temperature_raw"].astype(float)
                temperature = multinet.compute_temperature(
                    temperature_raw, multinet.calibrations["temperature"]
                )
                df["temperature"] = round(temperature, 3)

                conductivity_raw = df["conductivity_raw"].astype(float)
                conductivity = multinet.compute_conductivity(
                    conductivity_raw, multinet.calibrations["conductivity"]
                )
