...
class NetCdfHeader(OdfHeader):
    ...
    Attributes:
        _sytm_format: ``strptime``/``strftime`` format string used for
            ODF SYTM date/time values generated by this class.
    """
    ...
    def generate_creation_date(self) -> str:
        ...
        Returns:
            The current date/time, formatted per ``self._sytm_format``,
    ...
    def populate_parameter_headers(
        ...
        Maps each DataFrame column to its ODF parameter code (based on
        recognized Sea-Bird/NetCDF column-name conventions), looks up
        its description/units/print-format metadata via
        :func:`~datashop_toolbox.lookup_parameter.lookup_parameter`,
        builds a :class:`~datashop_toolbox.parameterhdr.ParameterHeader`
        for it, and appends it to ``self.parameter_headers``. Also
        updates ``self.data`` with the resulting parameter list, print
        formats, and (renamed) data frame.

        Args:
            df: Data to convert, with one column per NetCDF/Sea-Bird
                parameter name (e.g. ``"sytm"``, ``"temp"``, ``"sal"``).
            seabird_names: Unused; accepted for interface consistency
                but not referenced in this method.
            long_names: Unused; accepted for interface consistency but
                not referenced in this method.
            units: Unused; accepted for interface consistency but not
                referenced in this method.

        Returns:
            This :class:`NetCdfHeader` instance.
        """
        parameter_list = list()
        print_formats = dict()
