...
class NetCdfHeader(OdfHeader):
    ...
    def populate_parameter_headers(
        self, df: pd.DataFrame, seabird_names: list, long_names: list, units: list
    ) -> NoReturn:
        ...
        for idx, column in enumerate(df):
            ...
            if param == "SYTM":
                ...
                print_formats[param_code] = (
                    f"{parameter_header.get_print_field_width()}."
            ...
            parameter_header.set_print_field_width(
                parameter_info.get("print_field_width"), read_operation=True
            )
            parameter_header.set_print_decimal_places(
                parameter_info.get("print_decimal_places"), read_operation=True
            )
    ...
    def main():
        ...
        with contextlib.redirect_stdout(captured_output):
            ...
            top_folder = Path.join(drive, *pathlist)
            os.chdir(top_folder)

            netcdf_file = "cab054_2024_001.pcnv.nc"
            netcdf_path = Path.join(top_folder, netcdf_file)
            ...
            start_date = (
                datetime.strptime(sdate, "%Y-%m-%dT%H:%M:%SZ")
            ...
            end_date = (
                datetime.strptime(edate, "%Y-%m-%dT%H:%M:%SZ")
                .strftime(r"%d-%b-%Y %H:%M:%S")
                .upper()
                + ".00"
            )
            ...
            start_datetime = (
                datetime.strptime(sdatetime, "%Y-%m-%dT%H:%M:%S")
                .strftime(r"%d-%b-%Y %H:%M:%S")
                .upper()
                + ".00"
            )
            ...
            netcdf.event_header.set_event_comments(
                ds.__dict__["qa_applied"].replace("\n", ". "), read_operation=True
            )
            netcdf.event_header.set_event_comments(
                f"Sample Label Range: {ds.__dict__['stickers']}", read_operation=False
            )
            ...
            netcdf.instrument_header.set_serial_number(
                ds.__dict__["instrument"], read_operation=True
            )
            ...
            # print(df.head())
            # print(df.columns)
            # odf = netcdf.populate_parameter_headers(df, seabird_names, long_names, units)
