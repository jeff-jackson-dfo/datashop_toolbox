...
This test requires that the profile has pressures that are monotonically 
increasing (assuming the pressures are ordered from smallest to largest).
Action: If there is a region of constant pressure, all but the first of 
a consecutive set of constant pressures should be flagged as bad data. 
If there is a region where pressure reverses, all of the pressures in the 
reversed part of the profile should be flagged as bad data. All pressures 
flagged as bad data and all of the associated temperatures and salinities 
are removed from the TESAC distributed on the GTS.
