...
This method applies a fuzzy logic classification with a modified deffuzification 
in the end, and it was proposed in the following sequence of papers:

- Timms, G.P., de Souza, P.a., Reznik, L., Smith, D.V., 2011. 
Auto-mated data quality assessment of marine sensors. Sensors 11, 
9589–9602. doi:10.3390/s111009589.
- Morello, E., Lynch, T., Slawinski, D., Howell, B., Hughes, D., Timms, G., 2011. 
Quantitative quality control (qc) procedures for the australian national reference 
stations: Sensor data, in: OCEANS 2011, IEEE, Waikoloa, HI. pp. 1–7.
- Morello, E.B., Galibert, G., Smith, D., Ridgway, K.R., Howell, B., Slawin- ski, D., 
Timms, G.P., Evans, K., Lynch, T.P., 2014. Quality Control (QC) procedures for 
Australias National Reference Stations sensor dataComparing semi-autonomous systems 
to an expert oceanographer. Methods Oceanogr. 9, 17–33. doi:10.1016/j.mio.2014.09.001.
"""
...
def morello2014(features, cfg=None):
    ...
    if (cfg is None) or ("output" not in cfg) or ("features" not in cfg):
        ...
        cfg = {
            ...
            "features": {
                "spike": {"weight": 1, "low": {'type': 'zmf', 'params': [0.07, 0.2]}, 
                          "high": {'type': 'zmf', 'params': [2, 6]}},
                "woa_normbias": {"weight": 1, "low": {'type': 'zmf', 'params': [3, 4]}, 
                                 "high": {'type': 'zmf', 'params': [5, 6]}},
                "gradient": {"weight": 1, "low": {'type': 'zmf', 'params': [0.5, 1.5]}, 
                             "high": {'type': 'zmf', 'params':[3, 4]}},
            },
        }

    if not np.all([f in features for f in cfg["features"]]):
        module_logger.warning(
            f'Not all features ({cfg["features"].keys()}) required by morello2014 are available'
        )
        raise KeyError
